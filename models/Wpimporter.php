...
class Wpimporter extends Model
{
    ...
    public function onDoImport()
    {
        ...
        if (! empty($this->import_xml_file)) {
            ...
            $importXmlPath = $importXmlFile->getPath();
            ...
            $xmls = simplexml_load_string($importFileContent, "SimpleXMLElement", LIBXML_NOCDATA);
            ...
            foreach ($xmls->channel->item as $item) {
                try {
                    ...
                    if ($wpChildren->post_type == 'page' && ! empty($item->title)) {
                        ...
                        if ($defaultPostStatus == 'all' || $wpChildren->status == $defaultPostStatus) {
                            ...
                            $parsedown = new \Markdownify\Converter;
                            ...
                            if ($blogVersion == 'Radiantweb.Problog') {
                                ...
                                $postBlog->content = $parsedown->parseString($contentChildren->encoded);
                            }

                            $postBlog->title = $item->title;

                            //If post name empty for slug
                            $postBlog->slug = Str::slug($item->title);
